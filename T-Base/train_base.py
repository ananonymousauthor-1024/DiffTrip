...
"""
This model (T-Base) is a naive transformer-like model for trip planning, multiple steps
...
def main(opt):
    ...
    if opt.dataset_type == 'Flickr':
        ...
        with open(
                f'./results/T-Base/{opt.dataset}/{opt.lr}-{opt.batch_size}-{opt.d_model}-{opt.num_encoder_layers}-{opt.num_epochs}.txt',
                'w') as file:
